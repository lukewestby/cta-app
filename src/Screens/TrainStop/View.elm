module Screens.TrainStop.View exposing (view)
...
import Screens.TrainStop.Classes exposing (..)
...
viewPrediction routeName stopName prediction =
    let
        inMinutes =
            predictionInMinutes prediction
    in
...
viewPredictionGroup routeName stopName ( destination, predictions ) =
    div []
        [ div [ class [ DirectionLabel ] ] [ text destination ]
        , div []
            (predictions
                |> List.sortBy predictionInMinutes
                |> List.map (viewPrediction routeName stopName)
            )
        ]
    ...
    let
        items =
            model.predictions
                ...
                |> Dict.toList
                |> List.map (viewPredictionGroup model.stop.routeName model.stop.name)
    in
        div []
